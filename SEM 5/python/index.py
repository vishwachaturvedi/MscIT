...
def add_command():
    function.add(name.get(),user.get(),password.get(),category.get(),item.get(),amount.get())
    lb.delete(0,END)
    lb.insert(END,name.get(),user.get(),password.get(),category.get(),item.get(),amount.get())
...
l5 = Label(window,text="item")
l5.grid(row=4,column=0,columnspan=2)
l6 = Label(window,text="amount")
...
item=StringVar()
e5 = Entry(window,textvariable=item,width=50)
e5.grid(row=4,column=0,columnspan=10)

amount=StringVar()
e6 = Entry(window,textvariable=amount,width=50)
